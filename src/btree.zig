...
pub fn BTreeInternal(comptime T: usize) type {
    comptime {
        if (T < 2) @compileError("BTree order T must be >= 2");
    }
    ...
    return struct {
        ...
        pub const Order = T;
        pub const MaxKeys = MAX_KEYS;
        pub const MaxChildren = MAX_CHILDREN;
        pub const NodeType = Node;
        pub const Error = ValidateError;
        ...
        fn findKeyIdx(node: *Node, key: Key) usize {
            var lo: usize = 0;
            var hi: usize = node.n;
            while (lo < hi) {
                const mid = lo + (hi - lo) / 2;
                const mid_key = node.keys[mid];
                if (key > mid_key) {
                    lo = mid + 1;
                } else if (key < mid_key) {
                    hi = mid;
...
pub const BTree = BTreeInternal(DefaultOrder);
...
test "cursor iterate sorted" {
    ...
    while (cur.isValid()) {
        if (last) |prev| try testing.expect(prev <= cur.key());
        last = cur.key();
        count += 1;
        tree.cursorNext(&cur);
    }
...
test "cursorSeekGe works" {
    ...
    try testing.expect(cur.isValid());
    try testing.expectEqual(@as(Key, 5), cur.key());
